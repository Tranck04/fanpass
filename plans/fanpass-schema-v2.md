# FanPass — Schéma de base de données v2 (corrigé)

## Résumé des changements par rapport à la v1

| #   | Changement                                                                                                                                                    | Raison                                                         |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| 1   | `fan` : ajout `fan_profile` (solo/family/tourist/local/group/calm)                                                                                            | Nécessaire pour le matching                                    |
| 2   | `ticket` : ajout `event_id`, `tier_id`, `security_code`, `access_zone`, `tribune`, `access_control`, `access_rules`                                           | Supporte les billets fan zone/event + infos riches du frontend |
| 3   | `ticket_tier` : **nouvelle table**                                                                                                                            | Tiers tarifaires (Standard/Premium/VIP) par event              |
| 4   | `ticket_purchase` : **nouvelle table**                                                                                                                        | Historique d'achat de billets                                  |
| 5   | `event` : catégories `official/watch/sponsor/club/family`, ajout `density`, `capacity_pct`, `description`, `program`, `partners`, `merch_offer`, `route_hint` | Aligné avec le frontend                                        |
| 6   | `gate_plan` : **nouvelle table**                                                                                                                              | Plans d'itinéraire préconfigurés par gate                      |
| 7   | `fan_group` + `fan_group_member` : **nouvelles tables**                                                                                                       | Groupes de fans nommés avec capacité, mood, etc.               |
| 8   | `product` : catégories `official/club/souvenir/local/sponsor/post2030`, ajout `team`, `match_tag`, `event_tag`, `badge`, `promo`, `rating`, `pickup`          | Aligné avec le frontend                                        |
| 9   | `order` : ajout `pickup_type`                                                                                                                                 | Type de retrait (stade/fan zone/event)                         |
| 10  | `partner_service` + `partner_booking` : **nouvelles tables**                                                                                                  | Module Partenaires (VTC, hôtels, restos...)                    |
| 11  | `emergency_contact`, `safety_place`, `official_alert`, `support_request` : **nouvelles tables**                                                               | Module Safety                                                  |
| 12  | `itinerary` : référence `ticket_id` au lieu de `fan_id`+`match_id`                                                                                            | Lien direct ticket→itinéraire                                  |

## Couverture Vue ↔ Table après corrections

| Vue frontend             | Table(s)                                                                    | Couverture  |
| ------------------------ | --------------------------------------------------------------------------- | ----------- |
| 🎫 Billet                | `ticket` + `ticket_tier` + `ticket_purchase`                                | ✅ Complète |
| 🗺️ Parcours (itinéraire) | `itinerary` + `itinerary_step` + `gate_plan`                                | ✅ Complète |
| 🚪 Parcours (gate)       | `gate`                                                                      | ✅ Complète |
| 👥 Communauté (groupes)  | `fan_group` + `fan_group_member` + `fan_match_community`                    | ✅ Complète |
| ✨ Communauté (events)   | `event`                                                                     | ✅ Complète |
| 🛍️ Plus (merch)          | `product` + `order` + `order_item`                                          | ✅ Complète |
| 🆘 Plus (safety)         | `emergency_contact` + `safety_place` + `official_alert` + `support_request` | ✅ Nouveau  |
| 🤝 Plus (partenaires)    | `partner_service` + `partner_booking`                                       | ✅ Nouveau  |
...
CREATE TABLE ticket_tier (
    ...
    event_id        UUID NOT NULL,          -- FK vers match(id) OU event(id)
...
CREATE TABLE product (
    ...
    image_url    TEXT,
    visual_label VARCHAR(10),
    visual_from  VARCHAR(7),
    visual_to    VARCHAR(7),
...
-- Itinerary
CREATE INDEX idx_itinerary_ticket    ON itinerary(ticket_id);
CREATE INDEX idx_itinerary_fan       ON itinerary(fan_id);
...
-- ============================================================
-- SEED DATA — Mock pour développement
-- ============================================================
...
INSERT INTO event (stadium_id, match_id, name, category, subtitle, location_label, start_time, end_time, density, is_official) VALUES
  ('a1000000-0000-0000-0000-000000000001', 'b1000000-0000-0000-0000-000000000001',
   'Fan Zone A — Live Music', 'official', 'Fan zone officielle', 'Esplanade Nord', '17:00', '23:00', 'Dense', TRUE),
  ('a1000000-0000-0000-0000-000000000001', 'b1000000-0000-0000-0000-000000000001',
   'Watch Party Atlas', 'watch', 'Point de rencontre avant stade', 'Place Al Barid', '15:30', '17:00', 'Calme', FALSE),
  ('a1000000-0000-0000-0000-000000000001', 'b1000000-0000-0000-0000-000000000001',
   'Skills Challenge Sponsor', 'sponsor', 'Activation sponsor et mini-tournoi', 'Village Sponsors Nord', '12:00', '18:30', 'Controle', FALSE),
  ('a1000000-0000-0000-0000-000000000001', 'b1000000-0000-0000-0000-000000000001',
   'Lancement Maillot Maroc', 'club', 'Showcase officiel et précommande', 'FanPass Arena Pop-up', '19:00', '22:00', 'Controle', TRUE),
  ('a1000000-0000-0000-0000-000000000001', 'b1000000-0000-0000-0000-000000000001',
   'Family Ocean Stage', 'family', 'Familles, food court, live stats', 'Bouregreg Fan Park', '14:00', '23:30', 'Calme', FALSE),
  ('a1000000-0000-0000-0000-000000000001', 'b1000000-0000-0000-0000-000000000001',
   'Official FanPass Store', 'official', 'Boutique officielle', 'Près Gate D', '14:00', '23:00', 'Controle', TRUE);
